...
(function ($) {
	...
	fwSidebars = {
		...
		getSpecificPagesTabData: function () {
			...
			var possibleColors = {colors: 0};
			if (fwSidebars.getCurrentTab().find('.fw-ext-sidebars-positions select option:selected').length) {
				possibleColors = fwSidebars.getCurrentTab().find('.fw-ext-sidebars-positions select option:selected').data('extra-data');
			} else {
				possibleColors.colors = parseInt(fwSidebars.getCurrentTab().find('.fw-sidebars-count').val());
			}
		...
		changeImagePickerVal: function ($tab, value) {
			if (typeof $tab.find('.fw-ext-sidebars-positions select').data('picker') === 'object') {
		...
		showPositions: function () {
			...
			var possibleColors = {colors: 0};
			if ($currentTab.find('.fw-ext-sidebars-positions select option:selected').length) {
				possibleColors = $currentTab.find('.fw-ext-sidebars-positions select option:selected').data('extra-data');
			} else {
				possibleColors.colors = parseInt($currentTab.find('.fw-sidebars-count').val());
			}
